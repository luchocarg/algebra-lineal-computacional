...
from lab1 import error, error_relativo, esSimetrica, matricesIguales
...
assert sonIguales(
    np.float16(1), np.float16(1) + np.finfo("float16").eps, atol=1e-3
)
...
assert not matricesIguales(
    np.array([[1, 2], [3, 4]]).T, np.array([[1, 2], [3, 4]])
)
